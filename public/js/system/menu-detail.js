$(document).ready(function () {
    $('#add_role_menu').click(function () {
        ...
        if (roleId == '') {
            $('#role_id').addClass('has-warning');           
            $('#role_id').siblings('span:first').text('Menu is reqired');        
            return;
        }
        ...
        $.ajax({
            ...
            url: "/menus/" + $('#menu_id').val() + "/functions/add",
            data: {
                role_id: roleId,
                    ...
                    case 400:
                        ...
                        if (responseJSON.hasOwnProperty('result')) {
                            ...
                            if (result.hasOwnProperty('role_id')) {
                                var textHelpBlock = '';
                                result.role_id.forEach(element => {
                                    if (textHelpBlock == '')
                                ...
                                $('#role_id').addClass('has-warning');
                                $('#role_id').siblings('span:first').html(textHelpBlock);
                            }                            
                        }
                        break;
                    default:
