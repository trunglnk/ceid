<template>
  <div class="row">
    <div class="pr-4 pl-4 pt-4" style="display:flex; align-items:center; justify-content:space-between">
    ...
    <div class="pb-4 pl-4 pr-4" style="
      ...
      <div class="input-group">
        <input type="text" v-model="search" class="form-control" style="z-index: 0; width: 400px"
          placeholder="Tìm kiếm thông tin" v-on:keyup.enter="searchData()" />
        <span class="input-group-addon" style="cursor: pointer" @click="searchData()">
    ...
    <div class="nav-tabs-custom margin-top">
      <ul class="nav nav-tabs">
        <li class="active">
          <a href="#tab_dongbo155" data-toggle="tab" aria-expanded="true">Dữ liệu đã đồng bộ từ MTQG
          </a>
        </li>
        <li>
          <a href="#tab_khac155" data-toggle="tab" aria-expanded="false" v-if="dataChuaDongBo && dataChuaDongBo.length > 0">Dữ liệu khác</a>
        </li>
      </ul>
      <div class="tab-content">
        <div class="tab-pane active" id="tab_dongbo155">
          <table class="table table-hover fixed_headers">
            ...
            <tbody>
              <tr v-for="(item, index) in dataDongBo" :key="index">
                ...
                <td>{{ Number(item.phat_nho_nhat).toLocaleString('de-DE') }}</td>
                <td>{{ Number(item.phat_lon_nhat).toLocaleString('de-DE') }}</td>
          ...
          <div class="d-flex">
            <div>
              Hiển thị từ {{ (pagedongbo - 1) * perpage + 1 }} tới {{ ((pagedongbo * perpage) > totaldongbo) ?
                  totaldongbo : (pagedongbo * perpage)
              }} của
              {{ totaldongbo }}
            </div>
            <paginate v-model="pagedongbo" :page-count="pageCountdongbo" :page-range="3" :margin-pages="2"
              :click-handler="changePagedongbo" :prev-text="'‹‹'" :next-text="'››'" :container-class="'pagination'"
              :page-class="'page-item'">
            </paginate>
          </div>
        </div>
        <div class="tab-pane" id="tab_khac155" v-if="dataChuaDongBo && dataChuaDongBo.length > 0">
          <table class="table table-hover fixed_headers">
            <thead class="row-table-header">
              <tr class="row-table-header">
                <th>STT</th>
          ...
          <div class="d-flex">
            <div>
              Hiển thị từ {{ (page - 1) * perpage + 1 }} tới {{ ((page * perpage) > total) ? total :
                  (page * perpage)
              }} của
              {{ total }}
            </div>
    ...
    <modal-component width="450px" v-model="showModel" center @submit="deleteLoaiCoSo()"
      ...
      <div>Bạn có chắc chắn muốn xóa hành vi <b>{{ mahanhvi }}</b> ra khỏi các loại hình cơ sở không?</div>
    </modal-component>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getThoiGianDongBo() {
      axios
        .get(env.endpointhttp + "inthoigian/nghi_dinh_155")
        .then((response) => {
    ...
    dongbodulieu() {
      ...
      axios
        .get(env.endpointhttp + 'moitruongquocgia/dongbonghidinh155')
        .then((response) => {
          this.getHanhVi();
          this.getHanhViDongBo();
          this.getThoiGianDongBo();
    ...
    getHanhVi() {
      axios
        .get(env.endpointhttp + "getnghidinh155", {
          params: {
            page: this.page,
