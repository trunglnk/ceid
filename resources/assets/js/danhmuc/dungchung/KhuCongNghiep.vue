        ...
        <div class="row">
          ...
          <div class="col-md-4">
            ...
            <multiselect
              ...
              label="ten"
              track-by="id"
              :options="huyen.options"
              :multiple="true"
              selectLabel="Nhấn enter để chọn"
              deselectLabel="Nhấn enter để bỏ chọn"
              selectedLabel="Đã chọn"
              :loading="huyen.isLoading"
              :tabindex="3"
            >
            </multiselect>
          </div>
          <div class="col-md-4">
            ...
            <multiselect
              v-model="loaikhucongnghiep.value"
              placeholder="Chọn loại khu công nghiệp"
              label="ten"
              track-by="ma"
              :options="loaikhucongnghiep.options"
              :multiple="false"
              selectLabel="Nhấn enter để chọn"
              deselectLabel="Nhấn enter để bỏ chọn"
              selectedLabel="Đã chọn"
              :loading="loaikhucongnghiep.isLoading"
              :tabindex="4"
            >
            </multiselect>
          </div>
            ...
            <tr v-for="(item, index) in data" :key="item.data">
              ...
              <td>
                <input
                  ...
                  class="form-control  "
                  v-model="item.ten"
                  @change="updateIndustrialZone(item)"
                  v-if="
                    usersystem.role_code == 'admin' ||
                      usersystem.role_code == 'adminvaquanlydanhmuc'
                  "
              ...
              <td>
                <multiselect
                  ...
                  selectedLabel="Đã chọn"
                  :tabindex="3"
                  @input="updateIndustrialZone(item, 1)"
                  v-if="
                    usersystem.role_code == 'admin' ||
                      usersystem.role_code == 'adminvaquanlydanhmuc'
                  "
              ...
              <td>
                <multiselect
                  ...
                  selectedLabel="Đã chọn"
                  :tabindex="3"
                  @input="updateIndustrialZone(item)"
                  v-if="
                    usersystem.role_code == 'admin' ||
                      usersystem.role_code == 'adminvaquanlydanhmuc'
                  "
              ...
              <td>
                <multiselect
                  v-model="item.loai_khu_cong_nghiep"
                  placeholder="Chọn"
                  label="ten"
                  track-by="ma"
                  :options="loaikhucongnghiep.options"
                ...
                <input
                  type="text"
                  v-model="item.loai_khu_cong_nghiep.ten"
                  class="form-control"
                  v-else
                />
              </td>
    ...
    <div class="row">
      <div class="col-md-offset-9 col-md-3">
        <paginate
          ...
          :page-range="3"
          :margin-pages="2"
          :click-handler="getIndustrialZone"
          :prev-text="'‹‹'"
          :next-text="'››'"
  ...
  methods: {
    ...
    addIndustrialZone() {
      if (this.ten == "" || this.ten == null) {
        MessageService.showWarningMessage(
          "Bạn bắt buộc phải nhập tên và mã khu công nghiệp mới"
        );
      } else {
        axios
