<template>
  <div class="row">
    <div class="pt-4 pl-4 pr-4 d-flex">
      <div>
        <a
          href="https://moitruongquocgia.cebid.vn/#/group/csdl_mt/kho_so_bo?_view=T_ChuDauTu&_id=NULL&menu=thong_tin_chung"
        >
          <button
            type="submit"
            id="onsubmit"
            ...
            @click="goAdd"
          >
            <i class="fa fa-plus"></i> Thêm điểm trạm quan trắc
          </button>
        </a>
        <a href="/diemtramquantrac/add">
          <button
            type="submit"
            id="onsubmit"
            class="btn bg-olive btn-flat"
            @click="goAdd"
          >
            <i class="fa fa-plus"></i> Thêm điểm trạm quan trắc nội bộ
          </button>
        </a>
        <button
          class="btn bg-olive btn-flat"
            ...
            <table class="table table-hover fixed_headers">
              <thead class="row-table-header">
                <tr class="row-table-header">
                  ...
                  <th>Vĩ độ</th>
                  <th>Lưu vực sông</th>
                  <th>Loại điểm QTMT</th>
                  <th>Hành động</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(item, index) in DiemTramQTMT" :key="index">
                  ...
                  <td>{{ item.kinh_do }}</td>
                  <td>{{ item.vi_do }}</td>
                  <td>{{ item.luu_vuc_song_ten }}</td>
                  <td>{{ item.loai_diem_qtmt_ten }}</td>
        ...
        <div class="tab-pane" id="tab_khac" v-if="dataChuaDongBo">
          ...
          >
            <div>
              Hiển thị
              <select
                v-model="perpageChuaDongBo"
                style="width: 80px"
                @change="getDiemTramQTMT('chua_dong_bo')"
              >
                <option
                  v-for="(item, index) in options"
                  :value="item"
    ...
    <modal-component
      ...
      center
      @submit="xoa()"
      :title="'Xóa điểm trạm QTMT'"
    >
      <div>
        Bạn có chắc chắn muốn xóa điểm trạm
        <b>{{ xoaDiemTramQTMT.ten_goi }}</b> không ?
      </div>
    </modal-component>
  </div>
</template>
...
export default {
  ...
  created() {
    this.getDiemTramQTMT("da_dong_bo");
    this.getDiemTramQTMT("chua_dong_bo");
  ...
  methods: {
    searchData(trangThai = "da_dong_bo") {
      ...
      this.getDiemTramQTMT(trangThai);
    },
    getDiemTramQTMT(trangThai = "da_dong_bo") {
      axios
        .get(env.endpointhttp + "diemtramquantrac/get", {
          params: {
            page: trangThai == "da_dong_bo" ? this.page : this.pageChuaDongBo,
            perPage:
              trangThai == "da_dong_bo" ? this.perpage : this.perpageChuaDongBo,
            search: this.search,
            dong_bo: trangThai,
          },
        })
        .then((response) => {
          if (trangThai == "da_dong_bo") {
            this.DiemTramQTMT = response.data.data;
    ...
    goEdit(id) {
      window.location.href = "/diemtramquantrac/edit/" + id;
    },
    getThoiGianDongBo() {
      axios
        .get(env.endpointhttp + "inthoigian/diem_tram_qtmt")
        .then((response) => {
    ...
    dongbodulieu() {
      ...
      axios
        .get(env.endpointhttp + "moitruongquocgia/dongbodiemtramQTMT")
        .then((response) => {
          this.getDiemTramQTMT("da_dong_bo");
          this.getDiemTramQTMT("chua_dong_bo");
          this.getThoiGianDongBo();
        ...
        .catch((err) => {
          const message =
            ...
            "Không thể đồng bộ dữ liệu. Vui lòng kiểm tra trạng thái đồng bộ hoặc kết nối.";
          MessageService.showWarningMessage(message);
        })
    ...
    xoa() {
      axios
        .delete(
          env.endpointhttp +
            "diemtramquantrac/delete/" +
            this.xoaDiemTramQTMT.id
        )
        .then((response) => {
          this.getDiemTramQTMT("chua_dong_bo");
          this.getDiemTramQTMT("da_dong_bo");
    ...
    showConfirmDelete(diemTramQTMT) {
      this.showModel = true;
      this.xoaDiemTramQTMT = diemTramQTMT;
    },
    getTrangThaiDongBo() {
      axios
