...
export default {
  ...
  computed: {
    ...
    currentLoaiMoiTruong() {
      return this.loaiMoiTruongOptions.find((opt) => opt.value === this.loai);
    },
    activeMau() {
    ...
    currentFormData: {
      get() {
        if (Array.isArray(this.inputData) && this.inputData.length) {
          return (
            this.inputData.find((x) => x.loai_moi_truong === this.loai) || {
      ...
      set(newValue) {
        ...
        const idx = updated.findIndex((x) => x.loai_moi_truong === this.loai);

        if (idx === -1) updated.push(newValue);
        else updated[idx] = newValue;
