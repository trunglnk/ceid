@component('components.modals', [ 
    ...
    'title' => (empty($title) ? __('component.title_form_update') : __('component.title_form_update') . ' ' . $title),    
    'width' => (empty($width) ? '30%' : $width)])
<form class="form-horizontal" action="{{ route($route,empty($params)?$data->id:$params) }}" method="POST"  onsubmit="document.getElementById('{{'submit_'.$data->id}}').disabled=true">
    {{ csrf_field() }}    
