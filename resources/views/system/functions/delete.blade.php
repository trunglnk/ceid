@component('components.confirm-delete', [ 
    ...
    <div class="row">
        <div class="col-sm-12">
            <label for={{ 'inputParen_ID-detail-' . $rolemenu->id}} class="control-label">{{__('field.role')}}</label>
            <input type="text" class="form-control" id={{ 'inputParent_id-detail-' . $rolemenu->id}} value="{{$rolemenu->role->name}}" name="role_id" disabled>

            <label for={{ 'inputName-detail-' .$rolemenu->id}} class="control-label">{{__('field.menu')}}</label>
