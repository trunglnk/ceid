...
class KetQuaThanhTraChatThaiRanSinhHoat extends Model
{
    use ExecuteString;
    protected $fillable = [
    ...
    public function setKhoiLuongPhatSinhAttribute($value)
    {
        if (empty($value)) {
            $this->attributes['khoi_luong_phat_sinh'] = 0;
        } else {
            $this->attributes['khoi_luong_phat_sinh'] = str_replace(',', '', $value);
        }
    }

    public function gettKhoiLuongPhatSinhAttribute()
    {
        if (!empty($this->attributes['khoi_luong_phat_sinh'])) {
            return $this->attributes['khoi_luong_phat_sinh'];
        } else {
            return 0;
        }
    }
